...
if($dif_set!=''){
    ...
    echo $gen->get_diment().','.$gen->get_board();//should pass dimentions too, possibly s/f
    //setcookie('maze_gen', $gen->get_board(), time()+($game_time*1000));//define path please
}
else{
    echo 'failed get';
}
...
class Maze_mod{
    ...
    public function get_diment(){//return some of the maze definitions
        $str=$this->X_size.','.$this->Y_size.','.$this->Start.','.$this->Finish;
        return $str;
    }
